...
class HijriSyncService {
  ...
  Future<HijriSyncResult> syncHijriDate({
    ...
    try {
      ...
      if (day < 1 || day > _daysInOfficialMonth(record.year, record.month)) {
        return const HijriSyncResult(
          success: false,
          message:
      ...
      if (adjustment == null) {
        return HijriSyncResult(
          success: false,
          message:
              'Official date differs by more than one day. Keeping current date for safety.',
  ...
  int _daysInOfficialMonth(int year, int month) {
    final next = month == 12
        ? DateTime(year + 1, 1, 1)
        : DateTime(year, month + 1, 1);
    return next.difference(DateTime(year, month, 1)).inDays;
  }
